...
********************************************************************************
Author      :	Luis Majano , Sana Ullah
Date        :	04/05/2009
License		: 	Apache 2 License
Description :
Interacting with the Groovy Language.
...
<cfcomponent name="GroovyLoader" output="false" singleton>
	<cfproperty name="ocm" inject="cachebox:default">
	...
	<cffunction name="init" access="public" returntype="GroovyLoader">
		...
		variables.instance = {};
		/* requestService */
		instance.requestService = arguments.requestService;

		/* Core Java Lib Path where groovy lang is located */
		instance.javaLibPaths = arguments.settings.groovyLibPath;
		...
		if ( len( arguments.settings.libPath ) > 1 ) {
			instance.javaLibPaths = listAppend( instance.javaLibPaths, arguments.settings.libPath );
		}

		/* JavaLoad Groovy Lib & Extras */
		for ( var i = 1; i <= listLen( instance.javaLibPaths ); i++ ) {
			instance.javaLoader.appendPaths( expandPath( listGetAt( instance.javaLibPaths, i ) ) );
		}
	...
	>
		<cfscript>
		ocm.clearByKeySnippet( keySnippet = "groovy-", async = false );
		</cfscript>
	</cffunction>
	...
	>
		...
		var event   = instance.requestService.getContext();
